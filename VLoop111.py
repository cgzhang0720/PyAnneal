...
class VLoop111( DefectObject ) :
    'Class VLoop111 properties: 1. defecttype, 2. shape'
    ...
    def _set_emission( self, tmp_ConstNumber ) :
        ...
        Binding energy of one Vacancy to a Vacancy loop 111 of size nsize according to the following reaction 
        ...
        c2=22.41/(a0*a0*a0)
        Rc_vac(x)=a0*(x/(3.0**0.5*3.1415926))**0.5-delta_Rc
        formation energy of size x : vacloop111(x)=Rc_vac(x)*b*b*(c1_vac+c2*log(Rc_vac(x)))

        Final set of parameters            Asymptotic Standard Error
        =======================            ==========================
        c1_vac          = 1812.37          +/- 3.27         (0.1804%)
    ...
    def _set_shape_properties( self, jdata, tmp_ConstNumber ) :
        ...
        self.rho = tmp_ConstNumber.alatt * math.sqrt( self.nsize / ( math.sqrt( 3.0 ) * math.pi ) )
...
if __name__ == '__main__' :
    ...
    parser = argparse.ArgumentParser( description = '--- Class VLoop111 detecting ---' )
